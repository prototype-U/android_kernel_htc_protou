...
static void *protou_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init protou_init_wifi_mem(void)
{
	int i;
...
static struct platform_device protou_wifi_device = {
				.name           = "bcm4329_wlan",
				.id             = 1,
				.num_resources  = ARRAY_SIZE(protou_wifi_resources),
				.resource       = protou_wifi_resources,
				.dev            = {
				.platform_data = &protou_wifi_control,
				},
};

unsigned char *get_wifi_nvs_ram(void);

static unsigned protou_wifi_update_nvs(char *str)
{
...
int protou_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"protou_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	protou_wifi_update_nvs("sd_oobonly=1\n");
#endif
	protou_wifi_update_nvs("btc_params80=0\n");
	protou_wifi_update_nvs("btc_params6=30\n");
	protou_init_wifi_mem();
	ret = platform_device_register(&protou_wifi_device);
	return ret;
}