...
#define RAWCHIP_NORMAL_MODE
/* #define RAWCHIP_LECCI_BYPASS_MODE */
/* #define RAWCHIP_DTFILTER_BYPASS_MODE */
/* #define RAWCHIP_COLOR_BAR_MODE */
...
//	Yushan_ImageChar_t sImageChar;
	...
	uint32_t		udwIntrMask[] = {0x01E38E3B, 0xCC3C2C00, 0x001B7FFB};	// DXO_DOP_NEW_FR_PR enabled
	uint16_t		uwAssignITRGrpToPad1 = 0x008; /* Send only DOP interrupts to PAD1 */
       unsigned char EOTWord = 0x0f;
...
*/
	...
	if (strcmp(data.sensor_name, "s5k3h2yx") == 0)
	{
		sDxoDopTuning.bSharpness = 0x70;//woody0x01; /*0x60;*/
		sDxoDopTuning.bDenoisingLowGain = 0x75;//woody0x1; /* 0xFF for de-noise verify, original:0x1 */
	...
	else if (strcmp(data.sensor_name, "imx175") == 0)
	{
		sDxoDopTuning.bSharpness = 0x70;//woody0x01; /*0x60;*/
		sDxoDopTuning.bDenoisingLowGain = 0x75;//woody0x1; /* 0xFF for de-noise verify, original:0x1 */
		sDxoDopTuning.bDenoisingMedGain = 0x70;//imx175//0x60;//john0216//0x80;
		sDxoDopTuning.bDenoisingHiGain = 0x60;//imx175//0x40;//john0216//0x60; /*0x80;*/
	}
	else if (strcmp(data.sensor_name, "ov5693") == 0)
	{
		sDxoDopTuning.bSharpness = 0x60;//0x70;//woody0x01; /*0x60;*/
		sDxoDopTuning.bDenoisingLowGain = 0x90;//0x90;//0x75;//woody0x1; /* 0xFF for de-noise verify, original:0x1 */
		sDxoDopTuning.bDenoisingMedGain = 0x60;//0x75;//Mu L 0820 0x60;//imx175//0x60;//john0216//0x80;
		sDxoDopTuning.bDenoisingHiGain = 0x60;//0x75;//Mu L 0820 0x40;//imx175//0x40;//john0216//0x60; /*0x80;*/
	}
	else
	{
		sDxoDopTuning.bSharpness = 0x70;//woody0x01; /*0x60;*/
		sDxoDopTuning.bDenoisingLowGain = 0x75;//woody0x1; /* 0xFF for de-noise verify, original:0x1 */
		sDxoDopTuning.bDenoisingMedGain = 0x75;//woody0x60;//john0216//0x80;
		sDxoDopTuning.bDenoisingHiGain = 0x75;//woody0x40;//john0216//0x60; /*0x80;*/
	}

	sDxoDopTuning.bNoiseVsDetailsLowGain = 0x90;//woody0xA0;
	...
	/* Bypass EOT - for EVENT_RX_PHY_ERR_EOT_DL1/EVENT_RX_PHY_ERR_EOT_DL2 issue */
       SPI_Write(YUSHAN_MIPI_RX_EOT_BYPASS, 1, &EOTWord);
...
{
	uint8_t bStatus = SUCCESS;
	uint32_t		enableIntrMask[] = {0x01F38E3B, 0xCC3C2C00, 0x001B7FFB};
	uint32_t		disableIntrMask[] = {0x01E38E3B, 0xCC3C2C00, 0x001B7FFB};
